...
Subroutine gr3D
  ...
  !hist: arreglo para el histograma de las cintas r
  !NRcut: # de partículas estimado
  !inte: valor de la integral del trapecio
  real, dimension(:), allocatable, save :: integrando
  integer, dimension(:), allocatable, save :: hist
  real *4 NRcut, inte
  ...
  !generando arreglo para la integración de trapecio
  allocate(integrando(1:maxr))

  !generando arreglo para el histograma
  allocate(hist(1:maxr))

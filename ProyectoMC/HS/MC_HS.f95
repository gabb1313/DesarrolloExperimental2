...
! Valores de entrada
  ...
  !read *, N
1 write(*,*) 'Concentracion reducida'
  read *, rho
  !write(*,*) 'Numero de pasos'
  !read *, npasos
  !write(*,*) 'Desplazamiento maximo inicial'
  !read *, maxl
  ...
  maxl=0.5
  !iratio=isave
  NENER=1000                         !Configuración donde
...
! Calculamos la energía inicial de la partícula
   CALL ienerg3d(xiold,yiold,ziold,i,rcut,N,Vold)
...
! Calculamos la energía de la partícula después de movimiento
   CALL ienerg3d(xinew,yinew,zinew,i,rcut,N,Vnew)
